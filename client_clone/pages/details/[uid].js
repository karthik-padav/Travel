...
export default function Details(props) {
  ...
  const { details, nearByCategory = [], nearByLocation = [] } = { ...props };
  const { gpt } = { ...details };
  ...
  async function onLoad() {
    const longitude = details?.gmap?.geoJson?.coordinates?.[0];
    const latitude = details?.gmap?.geoJson?.coordinates?.[1];
    ...
    <>
      ...
      <div className="grid my-4 gap-4 px-10 grid-cols-4">
        <div className="col-span-3">
          ...
          {nearByCategory.length > 0 && (
            <section>
              <hr className="my-4" />
              <h3 className="text-black text-xl mb-2">
                Similar {category} to visit in {district_name}
              </h3>
              <ul className="list-disc pl-4">
                {nearByCategory.map((item) => {
                  const _latitude = item?.gmap?.geoJson?.coordinates?.[1];
                  const _longitude = item?.gmap?.geoJson?.coordinates?.[0];
                  const longitude = details?.gmap?.geoJson?.coordinates?.[0];
                  const latitude = details?.gmap?.geoJson?.coordinates?.[1];
                  if (!latitude || !longitude || !_latitude || !_longitude)
                    return null;
                  ...
                  return (
                    <li key={item.uid}>
                      ...
                      >
                        ...
                        <span>
                          {" "}
                          {meterToKm(distance)} km from {title}
                        </span>
                      </Link>
                    </li>
                  );
                          ...
                          >
                            <div className="h-52 relative rounded-lg overflow-hidden">
                              <Image
                                className="h-full"
                                src={
                                  item?.image || "/images/placeholder-image.jpg"
                                }
                                width={1000}
                                height={1000}
                                quality={100}
                            ...
                            <h3 className="pt-1 line-clamp-one ">
                              {item.title}
                            </h3>
                          </Link>
                ...
                <div key={item.id} className="col-span-1">
                  <div className="hover:shadow-md border rounded-lg p-1 h-full">
                    ...
                    >
                      ...
                      <p className="pt-1 text-sm line-clamp-one ">
                        {item.title}
                      </p>
                    </Link>
                  </div>
...
export async function getStaticProps(context) {
  ...
  if (uid) {
    ...
    const long = details?.gmap?.geoJson?.coordinates?.[0];
    const lat = details?.gmap?.geoJson?.coordinates?.[1];
    let promises = [];
    const category = details?.gmap?.category;
    const district_id = details?.location?.uid;
          ...
          const { data } = await client.query({
            ...
            variables: {
              where: {
                ...
                "location.uid": district_id,
              },
            },
          });
          res({ nearByCategory: data?.getThingsToDo?.data || [] });
        })
      );
    }
    if (long && lat) {
  ...
  return {
    props: {
      details,
      ...promiseResp.find((i) => i.nearByCategory),
      ...promiseResp.find((i) => i.nearByLocation),
    },
  };
}
